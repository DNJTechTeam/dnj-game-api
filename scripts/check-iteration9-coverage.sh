...
function in_service(file) {
  return file ~ /\/cmd\/loadtest\/(runner|report)\.go$/
}
...
END {
  ...
  printf "Iteration 9 loadtest-logic coverage: %.1f%% (%d/%d statements)\n", service_rounded, service_covered, service_total
  printf "Iteration 9 integrated coverage: %.1f%% (%d/%d statements)\n", slice_rounded, slice_covered, slice_total
