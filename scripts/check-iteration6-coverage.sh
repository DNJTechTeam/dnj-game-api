...
function in_service(file) {
  return file ~ /\/game_service\.go$/
}
function in_slice(file) {
  return in_service(file) ||
    file ~ /\/(game_handler|game_mapper|game_repository)\.go$/
}
NR > 1 {
  location($1)
  key = $1 " " $2
...
END {
  ...
  printf "Iteration 6 service coverage: %.1f%% (%d/%d statements)\n", service_rounded, service_covered, service_total
  printf "Iteration 6 integrated coverage: %.1f%% (%d/%d statements)\n", slice_rounded, slice_covered, slice_total
