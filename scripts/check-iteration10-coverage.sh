...
function in_service(file) {
  return file ~ /\/cmd\/handoff-check\/validate\.go$/
}
...
END {
  ...
  if (service_total == 0 || slice_total == 0) {
    print "Iteration 10 coverage gate matched zero statements — in_service/in_slice regex is stale or the profile is empty" > "/dev/stderr"
    exit 1
  }
  ...
  printf "Iteration 10 handoff-check coverage: %.1f%% (%d/%d statements)\n", service_rounded, service_covered, service_total
  printf "Iteration 10 integrated coverage: %.1f%% (%d/%d statements)\n", slice_rounded, slice_covered, slice_total
