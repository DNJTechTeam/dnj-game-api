...
function in_service(file) {
  return file ~ /\/notification_service\.go$/
}
function in_slice(file) {
  return in_service(file) ||
    file ~ /\/(notification_repository|notification_handler|notification_event_writer)\.go$/
}
NR > 1 {
  location($1)
  key = $1 " " $2
...
END {
  ...
  printf "Iteration 8 service coverage: %.1f%% (%d/%d statements)\n", service_rounded, service_covered, service_total
  printf "Iteration 8 integrated coverage: %.1f%% (%d/%d statements)\n", slice_rounded, slice_covered, slice_total
