...
const publicActivityProjection = `activities.id, activities.space_id, activities.slug, activities.name,
	...
	activities.check_in_points, activities.moment_points, activities.cooldown_seconds, activities.allows_moment,
...
func mapPublicActivityRow(row *publicActivityRow) activityEntities.PublicActivity {
	activity := activityEntities.Activity{ID: row.ID, SpaceID: row.SpaceID, Slug: row.Slug, Name: row.Name, Description: row.Description, Kind: activityEntities.Kind(row.Kind), Status: activityEntities.Status(row.Status), StartsAt: row.StartsAt, EndsAt: row.EndsAt, CheckInPoints: row.CheckInPoints, MomentPoints: row.MomentPoints, CooldownSeconds: row.CooldownSeconds, AllowsMoment: row.AllowsMoment, CreatedAt: row.CreatedAt, UpdatedAt: row.UpdatedAt}
