...
| 9 | Operação e carga | observabilidade final, segurança e soak/spike/stress baseados nos grafos reais de requests do frontend | perfis de carga, métricas e relatórios, sem novo domínio | Concluída localmente (smoke automatizado); soak/spike/stress em `develop` bloqueados por falta de acesso autorizado — PR aberto |
| 10 | Handoff final | OpenAPI publicado, página única de integração do frontend, manifesto, exemplos e checklist de release | documentação versionada e artefato publicado pelo CI | Concluída localmente; PR aberto para develop |
...
## Evidência local da Iteração 9

| Controle | Evidência em 2026-08-25 |
|---|---|
| Escopo | Sem domínio novo: transforma os grafos de request e a tabela de perfis já publicados em `docs/game-frontend-handoff.md` em uma ferramenta reproduzível (`cmd/loadtest`) e num gate real de CI, usando a observabilidade estruturada já existente (`RequestObservabilityMiddleware`) como fonte de verdade. |
| Ferramenta | `cmd/loadtest` aplica um perfil (concorrência, RPS, duração) contra um servidor em execução e falha (`exit 1`) quando `p95`, a taxa de erro agregada ou o teto de erros de servidor excedem o orçamento; requisições em voo não são canceladas no limite da janela (evita medir erro artificial); paceamento por agendamento absoluto (`início + i×intervalo`) que recupera atraso sob saturação em vez de perdê-lo. |
| Gate no CI | `make loadtest-smoke` sobe Postgres/MinIO local descartável via `docker compose` (sem credencial de produção), inicia o binário real da API, aguarda `/healthcheck`, roda o perfil **CI smoke reproduzível** (10 concorrência, 10 RPS, 2 min, timeout 5s, orçamento de erro agregado 0,5% e zero 5xx) e sempre encerra o servidor. Novo job `loadtest-smoke` em `.github/workflows/pr.yml`. Validado localmente: 1200 requisições, 0 erros, 0 erros de servidor, p95 1ms, max 3ms. |
| Cobertura | Gate novo `test-iteration9-cover-check`: 97,6% (82/84) sobre `cmd/loadtest/{report,runner}.go` — mínimo exigido 90%. `main()` fica fora do gate, como todo `cmd/*` deste repositório (`COVER_IGNORE_REGEX`). |
| Testes | `go test ./cmd/loadtest/... -race`: agregação de percentis/erro/erro-de-servidor, avaliação de orçamento (incluindo o teto independente de 5xx), execução contra `httptest.Server` (sucesso, erro HTTP, timeout de cliente, cancelamento de contexto pai, recuperação de atraso após saturação total de concorrência), serialização JSON em milissegundos. Sem flakiness observada em 10 execuções consecutivas sob `-race` (incluindo o novo teste de recuperação de atraso). |
| Achados de `/code-review` (nível high) corrigidos antes da publicação | (1) O relatório JSON serializava `time.Duration` como nanossegundos brutos nos campos `p50Ms`/`p95Ms`/`p99Ms`/`maxMs` — corrigido com `MarshalJSON` dedicado. (2) O paceamento usava um `time.Ticker` compartilhado lido diretamente pelos workers; como o canal do `time.Ticker` só bufferiza um tick pendente, uma saturação total e temporária de todos os workers descartava silenciosamente os ticks atrasados, reduzindo a taxa ofertada exatamente sob a condição que a ferramenta existe para flagrar — corrigido trocando por um único goroutine "pacer" que calcula o instante-alvo absoluto de cada requisição e recupera o atraso assim que um worker libera. (3) O perfil documentado como "0,5% e nenhum 5xx" só era enforced pela taxa agregada, sem checagem específica de zero 5xx — adicionados `ServerErrors`/`-max-server-errors` como um segundo orçamento independente, e `make loadtest-smoke` agora passa `-max-server-errors 0`. (4) `make loadtest-smoke` não repassava `-request-timeout-seconds`, rodando com o padrão de 8s da ferramenta em vez dos 5s documentados para o perfil — corrigido no `Makefile`/`scripts/run-loadtest-smoke.sh`. |
| Gate agregado | `test-iteration9-cover-check` adicionado a `make validate` e à linha de cobertura dos 4 workflows de CI (develop/pr/production/release); `loadtest-smoke` adicionado como job novo em `pr.yml`. |
| Bloqueio | Perfis de `develop` (soak/spike) e o perfil `canary` de produção, e todo perfil por fluxo autenticado (Game, scan, upload/galeria, notificações, etc.) da tabela em `docs/game-frontend-handoff.md` **não foram executados**: exigem acesso autorizado à infraestrutura real de `develop`/produção (URL, janela combinada com o time) e, para os fluxos autenticados, dados de conta — que esta execução não tem e não pode gerar em ambiente remoto por regra explícita. `cmd/loadtest` já suporta apontar para qualquer `-url`; falta autorização e execução manual, documentadas em `docs/load-testing.md`. |

## Evidência local da Iteração 10

| Controle | Evidência em 2026-08-25 |
|---|---|
| Escopo | Entrega obrigatória da Iteração 10 já registrada acima ("Entrega obrigatória da Iteração 10 para o frontend"): fonte canônica versionada, manifesto legível por máquina validado contra o OpenAPI, página HTML publicada com link visível na documentação V2, e artefato de workflow — todos gerados a partir da mesma fonte, nunca editados de forma independente. |
| Fonte canônica | `docs/handoff/DNJ-V2-FRONTEND-INTEGRATION.md`: checklist ordenado "faça isto", contrato comum (auth, erro, idempotência, paginação, UTC↔fuso-local), 4 helpers TypeScript de referência, tabela dos 24 fluxos (backlog granular com dependências, prioridade, estado, dono), detalhe dos 9 fluxos P0, matriz completa tela→operação→status→teste (67 linhas, uma por `operationId` publicado), estados vazios/retry, remoção segura de aliases, e checklist de aceite/rollback por fluxo. |
| Manifesto legível por máquina | `docs/handoff/dnj-v2-frontend-integration.json`: 24 fluxos cobrindo as 67 operações publicadas em exatamente 1:1 (nenhuma operação órfã, nenhuma duplicada — verificado por script e pelo validador Go abaixo). |
| Gate de CI (falha se divergir do OpenAPI) | `cmd/handoff-check` (novo): valida que todo `operationId` do manifesto de handoff existe em `docs/openapi/dnj-v2.operations.yaml`, que todo `operationId` publicado é coberto por exatamente um fluxo, e que cada fluxo tem os campos obrigatórios (id, screen, scope, state válido, owner, acceptanceTest, evidence; `blocked` exige `blockers` não vazio). Adicionado ao alvo `openapi` do `Makefile` (`make handoff-check`), portanto já roda em `make validate` e no job "OpenAPI Contract" de `pr.yml` sem mudança adicional de CI. |
| Cobertura | Gate novo `test-iteration10-cover-check`: 98,3% (59/60) sobre a lógica de validação (`cmd/handoff-check/validate.go`, isolada do `main()` do entrypoint em arquivo próprio, seguindo o mesmo padrão de exclusão de `main()` das Iterações 5–9) — mínimo exigido 90%. |
| Testes | `go test ./cmd/handoff-check/...`: 18 subtestes cobrindo cada violação de consistência (operação ausente do manifesto, operação não coberta, operação reivindicada por dois fluxos, id duplicado, estado/escopo inválido, `blocked` sem `blockers`, campo obrigatório ausente, dependência para id inexistente, dependência declarada antes do alvo no arquivo) e um teste que valida o manifesto e o OpenAPI reais deste repositório, não só fixtures. |
| Página HTML publicada | `docs/openapi/pages/frontend-integration.html`: página estática (sem dependência de CDN) que busca `dnj-v2-frontend-integration.json` em tempo de execução e renderiza o backlog; link visível adicionado em `docs/openapi/pages/v2-index.html` (a página principal da documentação V2). `scripts/publish-openapi-docs.sh` agora publica essa página + o JSON em `public/<env>/frontend-integration/`, substituindo o placeholder de referência do Markdown pelo SHA exato do commit implantado — testado localmente reproduzindo a chamada real do `develop.yml`/`production.yml` contra um ambiente de teste (gera `public/test-env/frontend-integration/{index.html,dnj-v2-frontend-integration.json}` corretamente). |
| Artefato do workflow | Passo novo "Upload frontend handoff artifact" (`actions/upload-artifact@v4`, retenção 90 dias) adicionado a `develop.yml` e `production.yml`, logo após a geração do site: empacota a página renderizada, o Markdown canônico, o manifesto JSON e o OpenAPI 3.0.3 completo — baixável direto do run do GitHub Actions. |
| Exemplos executáveis | Não foi criado um segundo conjunto de exemplos: os blocos `example` já existentes no OpenAPI (33, cobrindo identidade/perfil/jogos/moments/notificações) são a fonte única, renderizados pela Swagger UI já publicada — deliberado para não manter dois conjuntos de exemplos que poderiam divergir. |
| Checklist de release | `docs/implementation/release-checklist.md`: portão de conteúdo específico da V2 (PRs pendentes na ordem correta, todos os gates de cobertura incluindo o novo, migrations, contrato/documentação, perfis de carga não automatizados, segurança, rollback) a passar antes do passo mecânico de branch/tag em `docs/RELEASE.md`. |
| Gate agregado | `test-iteration10-cover-check` e `handoff-check` (via `openapi`) adicionados a `make validate` e aos 4 workflows de CI (develop/pr/production/release). |
| Bloqueio | A confirmação visual da página `/develop/frontend-integration/` e `/production/frontend-integration/` já publicadas e navegáveis no GitHub Pages depende de um deploy real de `develop.yml`/`production.yml`, que só roda em push para essas branches — fora do alcance desta execução (que não mergeia PRs). O script e a página foram validados localmente reproduzindo a chamada exata do workflow; falta apenas a confirmação em produção após o merge. |
